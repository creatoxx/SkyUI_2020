...
class SystemPage extends MovieClip
{
	...
	function OnShow(): Void
	{
		...
		//CategoryList.entryList.push({text: "$QUICKSAVE"}); //CrEaToXx: outcommented because no one should ever use quicksave!
	...
	function onLoad()
	{
		...
		SettingsList.entryList = [{text: "$Gameplay"}, {text: "$Display"}, {text: "$Audio"}];/*, {text: "$VR"}, {text: "$VR Performance"}]; //CrEaToXx: VR settings not needed!*/
		...
		if (iCurrentState == SystemPage.MAIN_STATE)
		{
			...
			switch (categoryName) {
				/*case "$QUICKSAVE":
					...
					//CrEaToXx: outcommented just to be sure if you hit the selector by accident?
  ...
  function OnSaveListCharactersOpenSuccess()
  {
	...
	function onSettingsCategoryPress(): Void
	{
		...
		switch (SettingsList.selectedIndex) {
			...
			//CrEaToXx: case 3 and 4 do not exist in non VR SSE, so they can be outcommented
	...
	//CrEaToXx: the following outcommented are Odie's VR adaption...I'm sure you could use that function if Schlangster would have uploaded the 5.2 source to the Github
	...
	//CrEaToXx: restored the original function...can't tell if the upper function is VR specific, or SkyUI relevant...everything's working perfectly...it seems it is not even needed, since it's not present in SkyUI2015?
	//it is present in the original SSE source however...so to me it is very clear at this point, that SkyUI SE is just a port of legendary, rather than completely new SE build
 
    function UpdatePermissions()
	{
		if(this._showModMenu)
		{
			gfx.io.GameDelegate.call("SetSaveDisabled",[this.CategoryList.entryList[0],
			this.CategoryList.entryList[0],
			this.CategoryList.entryList[1],
			this.CategoryList.entryList[2],
			this.CategoryList.entryList[4],
			this.CategoryList.entryList[6],false]);
		...
		else
		{
			...
			this.CategoryList.entryList[1],
			this.CategoryList.entryList[2],
			this.CategoryList.entryList[3],
			this.CategoryList.entryList[5],false]);
