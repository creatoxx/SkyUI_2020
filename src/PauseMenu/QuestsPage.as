...
class QuestsPage extends MovieClip
{
	...
	function startPage()
	{
		...
		if (bAllowShowOnMap)
			_showOnMapButton = _bottomBar.buttonPanel.addButton({text: "$Show on Map", controls: _showOnMapControls}); //CrEaToXx: this would be NMM specific to remove the Map access from quests page
	...
	function handleInput(details: InputDetails, pathToFocus: Array): Boolean
	{
		...
		if (GlobalFunc.IsKeyPressed(details)) {
			...
			{
				onShowMap(); //CrEaToXx: would be outcommented for NMM
				bhandledInput = true;
	...
	function SetPlatform(a_platform: Number, a_bPS3Switch: Boolean): Void
	{
		...
		/*_showOnMapControls = skyui.util.Input.pickControls(a_platform,
				{PCArt:"M",XBoxArt:"360_X",PS3Art:"PS3_X",ViveArt:"radial_Either_Up",MoveArt:"PS3_A",OculusArt:"OCC_A",WindowsMRArt:"radial_Either_Up"});*/ //removed access to map from quests page

		//CrEaToXx: restored the old way of catching the correct platform type
